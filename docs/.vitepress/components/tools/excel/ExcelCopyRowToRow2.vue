...
1、清理排水系淤泥体积：${formatFormula(aSheet, row.getCell(26).formula)}=${row.getCell(26).text}m³
2、清理灌木及杂草面积：${formatFormula(aSheet, row.getCell(28).formula)}=${row.getCell(28).text}m²
3、树木清砍：${row.getCell(27).text ? Math.ceil(Number(row.getCell(27))) + '株' : ''}
4、清理孤石：


二、投入情况

1、2025年2月13日能达公司投入${row.getCell(30).text}人${[
        { value: row.getCell(31).text, label: '100型防撞车' },
        { value: row.getCell(32).text, label: '60型挖机' },
        { value: row.getCell(33).text, label: '12t自卸车' },
        { value: row.getCell(34).text, label: '20t拖车' },
        { value: row.getCell(35).text, label: '大巴车' },
        { value: row.getCell(36).text, label: '施工车' },
      ]

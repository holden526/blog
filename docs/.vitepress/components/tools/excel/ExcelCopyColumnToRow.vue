...
const copyColumn = async () => {
  try {
    ...
    // 修改这部分，使用指定的工作表名
    const bSheet = workbookB.getWorksheet(pasteSheetName.value)
    if (bSheet) {
      // 拼接所有值，使用指定的间隔符
      if (copyType.value === 'formula') {
        bSheet.getCell(pastePosition.value).value = aFormula.join(separator.value)
      } else {
        const combinedValue = aVal.join(separator.value)
        bSheet.getCell(pastePosition.value).value = combinedValue
      }
    } else {
